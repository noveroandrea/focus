...
- **`src/extension/popup/Popup.tsx`** — 320px popup: activity status, per-page whitelist toggle, section pills, and Settings (idle time, icon-change heartbeats, beep volume/duration/style, AI classifier fields, allowed-domain editor, floating-companion toggle + always-on-top help). The header's expand button opens the dashboard in a tab.
