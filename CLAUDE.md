...
**Teams and competitions.** `team_members` is keyed `(user_id, team)` and `team_competitions` `(team, competition)` — the composite primary keys *are* the duplicate prevention, with no application check to forget. `teams`/`competitions` are thin registries so foreign keys work and so "create" (refuses an existing name) and "join" (refuses a missing one) can be different operations. Membership writes are `join_team` / `leave_team` / `enroll_team` / `leave_competition`. All are SECURITY INVOKER under RLS **except `join_team`**, which must read `teams.password_hash` — a column withheld from clients by column-level `GRANT SELECT (name, created_by, created_at)`, so RLS alone cannot express it. Teams carry a bcrypt password (create sets it, join must match); competitions deliberately do not.

`build_teams()` is the **only** place one participant's data reaches another, and defines the boundary: your teams' members, plus members of teams sharing a competition with yours, and nobody else. It is SECURITY DEFINER and therefore built to the safe shape — **no arguments**, caller from `auth.uid()`, `EXECUTE` revoked from `anon`/`PUBLIC`. Ranking everywhere is `focus + distracted`, because `distracted_score` is stored *negative* and that expression is "focus minus distraction"; the literal subtraction would rank a distracted user higher. Team totals are **sums** (so `member_count` ships alongside). Per-team lists inside a competition aren't sent separately — each member row carries its `team` and the popup groups by it.
